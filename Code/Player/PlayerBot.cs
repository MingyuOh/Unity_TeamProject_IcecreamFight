...
public class PlayerBot : Player
{
    ...
    [HideInInspector] public int currentIcecreamBall;
	[HideInInspector] public int currentCustomization;
    ...
    private void RandomPoint(Vector3 center, float range, out Vector3 result)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
			...
			Debug.LogFormat("RandomPoint: {0}", randomPoint);
		}
        ...
        else
        {
            ...
            for(int i = 0; i < inRange.Count; i++)
            {
                ...
                if (Physics.Linecast(transform.position, inRange[i].transform.position, out hit))
                {
					// 레이 테스트
					if (GetView().GetTeam() == 0)
						Debug.DrawRay(transform.position, inRange[i].transform.position * hit.distance, Color.red);
					else
						Debug.DrawRay(transform.position, inRange[i].transform.position * hit.distance, Color.blue);

					//get current enemy position and rotate this turret
					Vector3 lookPos = inRange[i].transform.position;
					transform.LookAt(lookPos);
					transform.eulerAngles = new Vector3(0, transform.eulerAngles.y, 0);
					playerRotation = (short)transform.eulerAngles.y;
                    Throw();
                    break;
                }
            }
